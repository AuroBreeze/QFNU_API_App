...
class ProxyHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if parsed.path == "/pyfa":
            ...
            if not session_id:
                _log("pyfa missing session id")
                self._send_json(400, {"error": "Missing sessionId"})
                return

            state = _get_session(session_id)
            if not state:
                _log(f"pyfa session expired sid={session_id[:8]}...")
                self._send_json(404, {"error": "Session expired"})
                return

            try:
                response = state.session.get(TRAINING_PLAN_URL, timeout=10)
            except requests.RequestException as exc:
                _log(f"pyfa request failed: {exc}")
                self._send_json(
                    502, {"error": "Training plan request failed", "detail": str(exc)}
                )
                return
